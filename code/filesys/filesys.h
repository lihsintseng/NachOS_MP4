...
class FileSystem {
  public:
	...
	OpenFileId OpenId(char *name){
	int fileDescriptor = OpenForReadWrite(name,FALSE);
		OpenFile *op=new OpenFile(fileDescriptor);
		fileDescriptorTable[fileDescriptor] = op;
...
class FileSystem {
  public:
	...
	bool RRemove(char *name, int place, int Nfirst);
    void List(char *name);			// List all the files in the file system
	void RList(char *name, int place, int Nfirst);
